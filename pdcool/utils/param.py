...
"""
Author            : revang
Date              : 2022-01-01 18:47:28
Last Modified by  : revang
Last Modified time: 2022-01-01 18:47:28
"""
...
def get_param(system, module, item):
    db = DBUtil()
    return db.queryone(
        f"select c_value from tparameter where c_system='{system}' and c_module='{module}' and c_item='{item}'"
    )[0]
...
def put_param(system, module, item, describe, value):
    db = DBUtil()
    return db.insert(
        f"""insert into tparameter(c_system,c_module,c_item,c_describe,c_value,c_createtime,c_updatetime) 
...
def post_param(system, module, item, value):
    db = DBUtil()
    return db.insert(
        f"""update tparameter set c_value='{value}',c_updatetime='{current_time()}'
...
def delete_param(system, module, item):
    db = DBUtil()
    return db.insert(
        f"""delete from tparameter where c_system='{system}' and c_module='{module}' and c_item='{item}'"""
    )
